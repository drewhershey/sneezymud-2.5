...
void raw_unlock_door(struct char_data *ch, struct room_direction_data *exitp, int door) {
  ...
  if (rp && (back = rp->dir_option[rev_dir[door]]) && back->to_room == ch->in_room) {
    ...
    sprintf(buf, "Inconsistent door locks in rooms %d->%d", ch->in_room, exitp->to_room);
    vlog(buf);
  }
}
...
int DisplayMove(struct char_data *ch, int dir, int was_in, int total) {
  ...
  for (tmp_ch = real_roomp(ch->in_room)->people; tmp_ch; tmp_ch = tmp_ch->next_in_room) {
    if (((!IS_AFFECTED(ch, AFF_SNEAK)) || (IS_IMMORTAL(tmp_ch))) && (CAN_SEE(tmp_ch, ch)) && (AWAKE(tmp_ch))) {
      if (tmp_ch != ch) {
        if (dir < 4) {
          sprintf(tmp, "%s has arrived from the %s.", PERS(ch, tmp_ch), dirs[rev_dir[dir]], total);
        } else if (dir == 4) {
          sprintf(tmp, "%s has arrived from below.", PERS(ch, tmp_ch), total);
        } else if (dir == 5) {
          sprintf(tmp, "%s has arrived from above.", PERS(ch, tmp_ch), total);
        } else {
          sprintf(tmp, "%s has arrived from somewhere.", PERS(ch, tmp_ch), total);
        }
        if (total > 1) {
...
void do_pick(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!HasClass(ch, CLASS_THIEF) && !HasClass(ch, CLASS_MONK) && (!IS_IMMORTAL(ch))) {
    ...
    for (door = 0; door <= 5; door++)
      if (EXIT(ch, door))
        if (EXIT(ch, door)->to_room != NOWHERE)
          if (!IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED) && IS_SET(real_roomp(EXIT(ch, door)->to_room)->room_flags, INDOORS)) {
            do_move(ch, "", ++door);
            return;
          }
...
void do_leave(struct char_data *ch, char *argument, int cmd) {
  ...
  if (!IS_SET(RM_FLAGS(ch->in_room), INDOORS))
    ...
    for (door = 0; door <= 5; door++)
      if (exit_ok(exitp = EXIT(ch, door), &rp) && !IS_SET(exitp->exit_info, EX_CLOSED) && !IS_SET(rp->room_flags, INDOORS)) {
        do_move(ch, "", ++door);
        return;
      }
          ...
          if (GET_POS(tmp_char) == POSITION_SLEEPING) {
            if (IS_AFFECTED(tmp_char, AFF_SLEEP)) {
              ...
              if (check_blackjack(tmp_char)) {
                act("You wake $M up and drag $m to $M feet.", FALSE, ch, 0, tmp_char, TO_CHAR);
                GET_POS(tmp_char) = POSITION_STANDING;
                act("You are awakened and drug to your feet by $n.", FALSE, ch, 0, tmp_char, TO_VICT);
              } else {
