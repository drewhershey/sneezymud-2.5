...
void spell_vitalize_mana(byte level, struct char_data *ch, struct char_data *victim, struct obj_data *obj) {
...
void spell_farlook(byte level, struct char_data *ch, struct char_data *victim, struct obj_data *obj) {
  sh_int target;
...
void spell_dispel_invisible(byte level, struct char_data *ch, struct char_data *victim, struct obj_data *obj) {
  struct affected_type af;
...
void spell_well_of_knowledge(byte level, struct char_data *ch, struct char_data *victim, struct obj_data *obj) {
  struct obj_data *o;
...
void spell_poly_self(byte level, struct char_data *ch, struct char_data *mob, struct obj_data *obj) {
  ...
  GET_NAME(mob) = buf;
  buf = (char *)malloc(strlen(mob->player.short_descr) + strlen(GET_NAME(ch)) + 2);
...
void spell_portal(byte level, struct char_data *ch, struct char_data *victim, struct obj_data *obj) {
  int location;
  char buf[30000];
  struct obj_data *tmp_obj;
  ...
  if (GetMaxLevel(victim) >= 50 || !rp || IS_SET(rp->room_flags, NO_SUM) || IS_SET(rp->room_flags, HAVE_TO_WALK) || IS_SET(rp->room_flags, NO_MAGIC)) {
      ...
      if (!in_group(ch, tmpv)) {
        if (IS_AFFECTED(tmpv, AFF_INVISIBLE)) {
          if (saves_spell(tmpv, SAVING_SPELL)) {
            ...
            act("$n is briefly revealed, but dissapears again.", TRUE, tmpv, 0, 0, TO_ROOM);
            act("You are briefly revealed, but dissapear again.", TRUE, tmpv, 0, 0, TO_CHAR);
