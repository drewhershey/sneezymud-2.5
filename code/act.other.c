...
void do_guard(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if (*argument == 0) {
    if (IS_SET(ch->specials.act, ACT_GUARDIAN)) {
      act("$n relaxes.", 0, ch, nullptr, nullptr, TO_ROOM);
      send_to_char("You relax.\n\r", ch);
      REMOVE_BIT(ch->specials.act, ACT_GUARDIAN);
    } else {
      SET_BIT(ch->specials.act, ACT_GUARDIAN);
      act("$n alertly watches you.", 0, ch, nullptr, ch->master, TO_VICT);
      act("$n alertly watches $N.", 0, ch, nullptr, ch->master, TO_NOTVICT);
    ...
    } else if (str_cmp(argument, "off") == 0) {
      if (IS_SET(ch->specials.act, ACT_GUARDIAN)) {
        act("$n relaxes.", 0, ch, nullptr, nullptr, TO_ROOM);
        send_to_char("You relax.\n\r", ch);
        REMOVE_BIT(ch->specials.act, ACT_GUARDIAN);
    ...
    while (num != 0) {
      ...
      if (tmp_object != nullptr) {
        ...
        if (count == 1) {
          ...
          act(buf, 1, ch, nullptr, nullptr, TO_CHAR);
...
void do_split(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if (is_number(buf) != 0) {
    ...
    if ((no_members != 0) && IS_AFFECTED(ch, AFF_GROUP)) {
...
void do_report(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  sprintf(info, "%d H, %d M. I am %s", (GET_HIT(ch)), (GET_MANA(ch)),
...
void do_quit(struct char_data* ch, const char* /*argument*/, int /*cmd*/) {
  if (IS_NPC(ch) || (ch->desc == nullptr) || IS_AFFECTED(ch, AFF_CHARM)) {
    return;
  }
  ...
  act("Goodbye, friend.. Come back soon!", 0, ch, nullptr, nullptr, TO_CHAR);
  act("$n has left the game.", 1, ch, nullptr, nullptr, TO_ROOM);
  zero_rent(ch);
...
void do_save(struct char_data* ch, const char* /*argument*/, int /*cmd*/) {
  ...
  if (IS_NPC(ch) && (IS_SET(ch->specials.act, ACT_POLYSELF))) {
    ...
    */
    if (ch->desc == nullptr) { /* fuck you assholes :-) */
      tmp = ch->orig;
    } else {
    ...
    */
    ...
    for (i = 0; i < MAX_WEAR; i++) {
      ...
      if ((ch->equipment[i] != nullptr) && ch->equipment[i]->in_room != -1) {
        o = ch->equipment[i];
...
void do_sneak(struct char_data* ch, const char* /*argument*/, int /*cmd*/) {
  ...
  if ((ch->skills[SKILL_SNEAK].learned == 0) &&
      (HasClass(ch, CLASS_THIEF) == 0) && ((HasClass(ch, CLASS_MONK)) == 0) &&
      (HasClass(ch, CLASS_ANTIPALADIN) == 0) && (!IS_IMMORTAL(ch))) {
...
void do_bload(struct char_data* ch, const char* arg, int /*cmd*/) {
  ...
  if ((arrow = get_obj_in_list_vis(ch, arrow_name, ch->carrying)) == nullptr) {
  ...
  act("You put the arrow into the $p", 1, ch, obj, nullptr, TO_CHAR);
  act("$n reloads the $p", 1, ch, obj, nullptr, TO_ROOM);
...
void do_reload(struct char_data* ch, const char* argument, int /*cmd*/) {
  struct obj_data* obj = nullptr;
  struct obj_data* shells = nullptr;
  ...
  if (obj->obj_flags.value[0] != ObjVnum(shells)) {
    sprintf(buf, "You can't get the %s to fit.\n\r", shells->short_description);
    send_to_char(buf, ch);
    return;
  }
  act("You reload the $p", 1, ch, obj, nullptr, TO_CHAR);
  act("$n reloads $p.", 1, ch, obj, nullptr, TO_ROOM);
...
void do_steal(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if ((str_cmp(obj_name, "coins") != 0) && (str_cmp(obj_name, "gold") != 0)) {
    if ((obj = get_obj_in_list_vis(victim, obj_name, victim->carrying)) ==
      ...
      for (eq_pos = 0; (eq_pos < MAX_WEAR); eq_pos++) {
        if ((victim->equipment[eq_pos] != nullptr) &&
            ((isname(obj_name, victim->equipment[eq_pos]->name)) != 0) &&
      ...
      if (obj == nullptr) {
        act("$E has not got that item.", 0, ch, nullptr, victim, TO_CHAR);
        return;
      } /* It is equipment */
      ...
      act("You unequip $p and steal it.", 0, ch, obj, nullptr, TO_CHAR);
      ...
      if ((ObjLevelCheck(obj, ch) == 0) ||
          (AWAKE(victim) && (percent > ch->skills[SKILL_STEAL].learned))) {
        ohoh = 1;
        act("Oops..", 0, ch, nullptr, nullptr, TO_CHAR);
        act("$n tried to steal something from you!", 0, ch, nullptr, victim,
          TO_VICT);
        act("$n tries to steal something from $N.", 1, ch, nullptr, victim,
          TO_NOTVICT);
        if (IS_PC(victim)) {
    ...
    if (IS_PC(victim) ||
      ...
      act("You discover that $n has $s hands in your wallet.", 0, ch, nullptr,
        victim, TO_VICT);
      act("$n tries to steal gold from $N.", 1, ch, nullptr, victim,
        TO_NOTVICT);
    } else {
  ...
  if ((ohoh != 0) && IS_NPC(victim) && AWAKE(victim)) {
    ...
    case 'm': {
      ...
      for (i = 0; *spells[i] != '\n' && i < MAX_SKILLS - 1; i++) {
        if ((spell_info[i + 1].spell_pointer != nullptr) &&
          ...
          sprintf(buf, "[%d] %s %s \n\r", spell_info[i + 1].min_level_magic,
...
void do_idea(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if ((fl = fopen(IDEA_FILE, "a")) == nullptr) {
    perror("do_idea");
...
void do_typo(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if (GetMaxLevel(ch) == 60) {
    if (ch != nullptr) {
      start_page_file(ch->desc, TYPO_FILE,
        "The players cant spell worth shit.\n\r");
      return;
    }
  }
  ...
  if ((fl = fopen(TYPO_FILE, "a")) == nullptr) {
    perror("do_typo");
...
void do_bug(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if ((fl = fopen(BUG_FILE, "a")) == nullptr) {
    perror("do_bug");
...
void do_monitor(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if (isdigit(*num) == 0) {
    ...
    send_to_char("X fits in the range 1-X, which is the number of channels",
      ch);
...
void do_channel(struct char_data* ch, const char* argument, int /*cmd*/) {
  struct obj_data* radio = nullptr;
  ...
  if (*num == 0) {
    for (i = descriptor_list; i != nullptr; i = i->next) {
      if ((i->character != nullptr) && ((CAN_SEE(ch, i->character)) != 0) &&
          (i->connected == CON_PLYNG) && (i->character->in_room != NOWHERE)) {
        radio2 = i->character->equipment[WEAR_RADIO];

        if (radio2 != nullptr) {
          sprintf(buf, "%-15s : Channel %d\n\r", GET_NAME(i->character),
            radio2->obj_flags.value[3]);
          send_to_char(buf, ch);
        }
      }
    }
...
void do_prompt(struct char_data* ch, const char* arg, int /*cmd*/) {
  ...
  if (strlen(arg) > 40) {
    ...
    ch->desc->prompt = nullptr;
  }
}

void do_terminal(struct char_data* ch, const char* argument, int /*cmd*/) {
  char buf[80];
  char term[80];
  char screen[80];
  ...
  if (*term != 0) {
    if (is_abbrev(term, "screensize") != 0) {
      if (*screen != 0) {
        if (isdigit(*screen) != 0) {
          if (atoi(screen) == 0) {
...
void do_cls(struct char_data* ch, const char* /*argument*/, int /*cmd*/) {
  char buf[80];
...
void do_log(struct char_data* ch, const char* argument, int /*cmd*/) {
...
void do_color(struct char_data* ch, const char* /*argument*/, int /*cmd*/) {
  ...
  if (IS_SET(ch->specials.act, PLR_COLOR)) {
    ...
    sprintf(buf, "%sC%so%sl%so%sr%s mode on.\n\r", ANSI_RED, ANSI_CYAN,
      ANSI_BLUE, ANSI_ORANGE, ANSI_VIOLET, ANSI_NORMAL);
    send_to_char(buf, ch);
...
void do_group(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if (*name == 0) {
    if (!IS_AFFECTED(ch, AFF_GROUP)) {
      ...
      for (f = k->followers; f != nullptr; f = f->next) {
        if (IS_AFFECTED(f->follower, AFF_GROUP)) {
          sprintf(buf, "%-15s [%.1f%%dhp %.1f%%m. %s looks %s.\n\r",
            (IS_NPC(f->follower) ? f->follower->player.short_descr
                                 : GET_NAME(f->follower)),
            (((float)(GET_HIT(f->follower))) /
              ((float)GET_MAX_HIT(f->follower)) * 100),
            (((float)(GET_MANA(f->follower))) /
              ((float)GET_MAX_MANA(f->follower)) * 100),
            (IS_NPC(f->follower) ? f->follower->player.short_descr
                                 : GET_NAME(f->follower)),
            DescMoves(((float)GET_MOVE(f->follower)) /
                      ((float)GET_MAX_MOVE(f->follower))));
          send_to_char(buf, ch);
        }
      }
    }
  ...
  if ((victim = get_char_room_vis(ch, name)) == nullptr) {
    ...
    if (victim == ch) {
      ...
      for (f = ch->followers; f != nullptr; f = f->next) {
    ...
    if (found != 0) {
      if (IS_AFFECTED(victim, AFF_GROUP)) {
        act("$n has been kicked out of $N's group!", 0, victim, nullptr, ch,
          TO_ROOM);
        act("You are no longer a member of $N's group!", 0, victim, nullptr, ch,
          TO_CHAR);
        REMOVE_BIT(victim->specials.affected_by, AFF_GROUP);
      } else {
        if (GetMaxLevel(victim) >= LOW_IMMORTAL) {
          act("You really don't want $n in your group.", 0, ch, nullptr,
            nullptr, TO_CHAR);
          return;
        }
        if (GetMaxLevel(ch) >= LOW_IMMORTAL) {
          act("Now now.  That would be CHEATING!", 0, ch, nullptr, nullptr,
            TO_CHAR);
          return;
        }
        act("$n is now a member of $N's group.", 0, victim, nullptr, ch,
          TO_ROOM);
        act("You are now a member of $N's group.", 0, victim, nullptr, ch,
          TO_CHAR);
        SET_BIT(victim->specials.affected_by, AFF_GROUP);
      }
    } else {
      act("$N must follow you, to enter the group", 0, ch, nullptr, victim,
        TO_CHAR);
    }
  }
}
...
void do_quaff(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if ((temp = get_obj_in_list_vis(ch, buf, ch->carrying)) == nullptr) {
    ...
    equipped = 1;
    if ((temp == nullptr) || (isname(buf, temp->name) == 0)) {
      act("You do not have that item.", 0, ch, nullptr, nullptr, TO_CHAR);
      return;
    }
  }
  ...
  if (temp->obj_flags.type_flag != ITEM_POTION) {
    act("You can only quaff potions.", 0, ch, nullptr, nullptr, TO_CHAR);
    return;
  }

  act("$n quaffs $p.", 1, ch, temp, nullptr, TO_ROOM);
  act("You quaff $p which dissolves.", 0, ch, temp, nullptr, TO_CHAR);

  /*  my stuff */
  if (ch->specials.fighting != nullptr) {
    if (equipped != 0) {
      if (number(1, 20) > ch->abilities.dex) {
        act("$n is jolted and drops $p!  It shatters!", 1, ch, temp, nullptr,
          TO_ROOM);
        act("You arm is jolted and $p flies from your hand, *SMASH*", 1, ch,
          temp, nullptr, TO_CHAR);
  ...
  for (i = 1; i < 4; i++) {
    if (temp->obj_flags.value[i] >= 1) {
      ((*spell_info[temp->obj_flags.value[i]].spell_pointer)(
        (signed char)temp->obj_flags.value[0], ch, "", SPELL_TYPE_POTION, ch,
        temp));
    }
  }
...
void do_recite(struct char_data* ch, const char* argument, int /*cmd*/) {
  char buf[100];
  struct obj_data* scroll = nullptr;
  struct obj_data* obj = nullptr;
  struct char_data* victim = nullptr;
  int i = 0;
  int bits = 0;
  ...
  if ((scroll = get_obj_in_list_vis(ch, buf, ch->carrying)) == nullptr) {
    scroll = ch->equipment[HOLD];
    equipped = 1;
    if ((scroll == nullptr) || (isname(buf, scroll->name) == 0)) {
      act("You do not have that item.", 0, ch, nullptr, nullptr, TO_CHAR);
      return;
    }
  }
  ...
  if (scroll->obj_flags.type_flag != ITEM_SCROLL) {
    act("Recite is normally used for scrolls.", 0, ch, nullptr, nullptr,
      TO_CHAR);
    return;
  }
  ...
  act("$n recites $p.", 1, ch, scroll, nullptr, TO_ROOM);
  act("You recite $p which bursts into flame.", 0, ch, scroll, nullptr,
    TO_CHAR);
  ...
  for (i = 1; i < 4; i++) {
    if (scroll->obj_flags.value[i] >= 1) {
      ...
      ((*spell_info[scroll->obj_flags.value[i]].spell_pointer)(
        (signed char)scroll->obj_flags.value[0], ch, "", SPELL_TYPE_SCROLL,
        victim, obj));
    }
  }
  if (equipped != 0) {
...
void do_use(struct char_data* ch, const char* argument, int /*cmd*/) {
  ...
  if (ch->equipment[HOLD] == nullptr ||
      (isname(buf, ch->equipment[HOLD]->name) == 0)) {
    act("You do not hold that item in your hand.", 0, ch, nullptr, nullptr,
      TO_CHAR);
    return;
  }
  ...
  if (stick->obj_flags.type_flag == ITEM_STAFF) {
    act("$n taps $p three times on the ground.", 1, ch, stick, nullptr,
      TO_ROOM);
    act("You tap $p three times on the ground.", 0, ch, stick, nullptr,
      TO_CHAR);
    if (stick->obj_flags.value[2] > 0) { /* Is there any charges left? */
      ...
      ((*spell_info[stick->obj_flags.value[3]].spell_pointer)(
        (signed char)stick->obj_flags.value[0], ch, "", SPELL_TYPE_STAFF,
        nullptr, nullptr));
      WAIT_STATE(ch, PULSE_VIOLENCE);
    } else {
    ...
    bits = generic_find(argument,
      ...
      &tmp_char, &tmp_object);
    if (bits != 0) {
      struct spell_info_type* spellp = nullptr;
...
void do_teams(struct char_data* ch, const char* /*argument*/, int /*cmd*/) {
  char buf[255];
  ...
  for (d = descriptor_list; d != nullptr; d = d->next) {
    if ((d->connected == 0) &&
        ((real_roomp(
           (person = ((d->original != nullptr) ? d->original : d->character))
             ->in_room)
