...
static void lose_dice(struct char_data* ch) {
  ...
  if ((crap_man != nullptr) && (dice != nullptr)) {
...
static int check_pointroll() {
  ...
  for (tmp_char = character_list; tmp_char != nullptr; tmp_char = temp) {
    temp = tmp_char->next;
...
int check_slot_player(struct char_data* ch) {
  ...
  for (better = character_list; better != nullptr; better = temp) {
    temp = better->next;
  ...
  if (*craps != 0) {
    if (isdigit(*craps) != 0) {
      ...
      if (is_abbrev(amount, "crap") != 0) {
        ...
        GET_GOLD(ch) -= num;
        ch->bet.crap += num;
      } else if (is_abbrev(amount, "come") != 0) {
        get_dice(ch);
        if (ch->bet.come == 0) {
        ...
        GET_GOLD(ch) -= num;
        ch->bet.come += num;
      } else if (is_abbrev(amount, "three") != 0) {
        if (ch->bet.three == 0) {
          ch->bet_opt.one_roll += THREE3;
          sprintf(buf,
            "You just placed %d gold down on a oneroll three (3) bet\n\r", num);
        } else {
          sprintf(buf,
            "You add %d gold to your bet on the oneroll three (3).\n\r", num);
        }
        send_to_char(buf, ch);
        GET_GOLD(ch) -= num;
        ch->bet.three += num;
      } else if (is_abbrev(amount, "two") != 0) {
        if (ch->bet.two == 0) {
          ch->bet_opt.one_roll += TWO2;
          sprintf(buf,
            "You just placed %d gold down on a oneroll two (2) bet\n\r", num);
        } else {
          sprintf(buf,
            "You add %d gold to your bet on the oneroll two (2).\n\r", num);
        }
        send_to_char(buf, ch);
        GET_GOLD(ch) -= num;
        ch->bet.two += num;
      } else if (is_abbrev(amount, "eleven") != 0) {
        if (ch->bet.eleven == 0) {
          ch->bet_opt.one_roll += ELEVEN;
          sprintf(buf,
            "You just placed %d gold down on a oneroll eleven (11) bet.\n\r",
            num);
        } else {
          sprintf(buf,
            "You add %d gold to your bet on the oneroll eleven (11).\n\r", num);
        }
        send_to_char(buf, ch);
        GET_GOLD(ch) -= num;
        ch->bet.eleven += num;
      } else if (is_abbrev(amount, "twelve") != 0) {
        if (ch->bet.twelve == 0) {
          ch->bet_opt.one_roll += TWELVE;
          sprintf(buf,
            "You just placed %d gold down on a oneroll twelve (12) bet.\n\r",
            num);
        } else {
          sprintf(buf,
            "You add %d gold to your bet on a oneroll twelve (12) bet.\n\r",
            num);
        }
        send_to_char(buf, ch);
        GET_GOLD(ch) -= num;
        ch->bet.twelve += num;
      } else if (is_abbrev(amount, "horn") != 0) {
        if (ch->bet.horn_bet == 0) {
          ch->bet_opt.one_roll += HORN_BET;
          sprintf(buf,
            "You just placed %d gold down on a oneroll horn bet.\n\r", num);
        } else {
        ...
        GET_GOLD(ch) -= num;
        ch->bet.horn_bet += num;
      } else if (is_abbrev(amount, "field") != 0) {
        if (ch->bet.field_bet == 0) {
          ch->bet_opt.one_roll += FIELD_BET;
          sprintf(buf,
            "You just placed %d gold down on a oneroll field bet.\n\r", num);
        } else {
          sprintf(buf,
            "You add %d gold to your bet on a oneroll field bet.\n\r", num);
        }
        send_to_char(buf, ch);
        GET_GOLD(ch) -= num;
        ch->bet.field_bet += num;
      } else if (is_abbrev(amount, "seven") != 0) {
        if (ch->bet.seven == 0) {
          ch->bet_opt.one_roll += SEVEN;
          sprintf(buf,
            "You just placed %d gold down on a oneroll seven (7) bet.\n\r",
            num);
        } else {
          sprintf(buf,
            "You add %d gold to your bet on a oneroll seven (7) bet.\n\r", num);
        }
        send_to_char(buf, ch);
        GET_GOLD(ch) -= num;
        ch->bet.seven += num;
...
static void spin_slot(struct char_data* ch) {
  ...
  if ((strcmp(fruit3, "bally") == 0) && (fruit2 == fruit1)) {
    send_to_char("You win!\n\r", ch);
...
static void check_oneroll_seven(struct char_data* better, int diceroll) {
  if (diceroll == 7) {
...
static void set_point(struct char_data* ch, int diceroll) {
  char buf[255];
...
static void roll_dice(struct char_data* ch) {
  ...
  if (table_man != nullptr) {
    if (can_bet_craps(ch) == 0) {
...
void do_play(struct char_data* ch, const char* arg, int cmd) {
  ...
  if (is_abbrev(game, "slots") != 0) {
    ...
    if (*options == 0) {
      ...
      send_to_char("3) Play the cheap slots, and bet 3 coins (300 coins).\n\r",
        ch);
      send_to_char(
        "4) Play the expensive slots and bet 1 coin (1000 mud coins.\n\r", ch);
      send_to_char(
        "5) Play the expensive slots and bet 2 coins (2000 mud coins.\n\r", ch);
      send_to_char(
        "6) Play the expensive slots and bet 3 coins (3000 mud coins.\n\r", ch);
      send_to_char(
        "You MUST play the 3000 coins machine to have a chance at the "
        "jackpot\n\r",
        ch);
    } else {
      if (strcmp(options, "1") == 0) {
        if (GET_GOLD(ch) >= 100) {
          ...
          send_to_char(
            "You search your pockets for coins but come up emtpy-handed.\n\r",
            ch);
          return;
        }
  ...
  } else if (is_abbrev(game, "roulette") != 0) {
    if (*options == 0) {
      send_to_char(R_TABLE, ch);
      return;
    }
  } else if (is_abbrev(game, "craps") != 0) {
    if (*options == 0) {
...
int craps_table_man(struct char_data* ch, int cmd, const char* arg) {
  ...
  if (cmd != 0) {
    ...
    if (cmd == 274) {
      ...
      if (check_pointroll() != 0) {
        if ((is_abbrev(options, "come") != 0) ||
            (is_abbrev(options, "craps") != 0)) {
  ...
  } else if (check_for_dice_in_inv(ch) != 0) {
    if (number(1, 9) == 1) {
