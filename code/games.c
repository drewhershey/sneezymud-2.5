...
char *ChooseFirstFruit() {
  ...
  static char *fruits[8] = {
    "cherry", "lime", "orange", "apple", "banana", "bell", "bally", "Seven",
  };
  num = number(1, 15);
...
void set_point(struct char_data *ch, int diceroll) {
  ...
  if ((diceroll == 2) || (diceroll == 3) || (diceroll == 7) || (diceroll == 11) || (diceroll == 12))
    return;
...
void check_field(struct char_data *better, int diceroll) {
  ...
  if ((diceroll >= 5) && (diceroll <= 8)) {
    if (crap_man) {
      sprintf(buf, "%s The roll is %d. You lose your bet on the field.", GET_NAME(better), diceroll);
      do_tell(crap_man, buf, 0);
    }
  } else {
    if (crap_man) {
      sprintf(buf, "%s The roll is %d. You win your bet on the field!", GET_NAME(better), diceroll);
      do_tell(crap_man, buf, 0);
    }
...
void roll_dice(struct char_data *ch) {
  ...
  if (table_man) {
    ...
    if (ch->bet.come == 0) {
      sprintf(buf, "%s Sorry to keep the table, you need to place a come bet.", GET_NAME(ch));
      do_tell(table_man, buf, 0);
      return;
    }
  }
...
void spin_slot(struct char_data *ch) {
  ...
  } else if (strcmp(fruit1, "Seven") && (fruit1 == fruit2) && (fruit2 == fruit3)) {
    send_to_char("You win!\n\r", ch);
...
int craps_table_man(struct char_data *ch, int cmd, char *arg) {
  ...
  if (cmd) {
    if ((cmd != 0) && (cmd != 1) && (cmd != 2) && (cmd != 3) && (cmd != 4) && (cmd != 5) && (cmd != 274) && (cmd != 280)) {
      return (FALSE);
    }
    ...
    if (cmd == 274) {
      ...
      if (check_pointroll(ch)) {
        if (is_abbrev(options, "come") || is_abbrev(options, "craps")) {
          sprintf(buf, "Sorry %s, no bets can be placed on the come", GET_NAME(ch));
          do_say(crap_man, buf, 0);
