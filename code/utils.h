...
#define GET_CLASS_TITLE(ch, class, lev) \
  ((ch)->player.sex ? (((ch)->player.sex == 1) ? titles[(class)][(lev)].title_m : titles[(class)][(lev)].title_f) \
...
#define GET_REQ(i) \
  (i < 2 ? "Awful" \
         : (i < 4 ? "Bad" \
                  : (i < 7 ? "Poor" \
                           : (i < 10 ? "Average" \
                                     : (i < 14 ? "Fair" : (i < 20 ? "Good" : (i < 24 ? "Very good" : "Superb")))))))

#define GET_WEAPON_DAMAGE(i) \
  (i < 2 ? "Very low damage" \
         : (i < 5 ? "Low damage" \
                  : (i < 10 ? "Moderate damage" \
                            : (i < 15 ? "A good bit of damage" : (i < 20 ? "Very nice damage" : "Superb Damage")))))
...
#define AWAKE(ch) (GET_POS(ch) > POSITION_SLEEPING && !IS_AFFECTED(ch, AFF_PARALYSIS))
...
#define CAN_SEE_OBJ(sub, obj) \
  (((!IS_NPC(sub)) && (GetMaxLevel(sub) > LOW_IMMORTAL)) || \
   (((!IS_SET((obj)->obj_flags.extra_flags, ITEM_INVISIBLE) || IS_AFFECTED((sub), AFF_DETECT_INVISIBLE)) && \
...
#define CAN_CARRY_N(ch) \
  (((ch)->equipment[WIELD] && (ch)->equipment[WEAR_SHIELD]) \
     ...
     : (((ch)->equipment[WIELD] || (ch)->equipment[WEAR_SHIELD]) ? (dex_app_skill[GET_DEX(ch)].volume) / 2 \
...
#define IS_POLICE(ch) \
  ((mob_index[ch->nr].virtual == 3060) || (mob_index[ch->nr].virtual == 3069) || (mob_index[ch->nr].virtual == 14) || \
   (mob_index[ch->nr].virtual == 15) || (mob_index[ch->nr].virtual == 16) || (mob_index[ch->nr].virtual == 17) || \
   (mob_index[ch->nr].virtual == 18) || (mob_index[ch->nr].virtual == 19) || (mob_index[ch->nr].virtual == 3067))

#define IS_CORPSE(obj) (GET_ITEM_TYPE((obj)) == ITEM_CONTAINER && isname("corpse", (obj)->name))
...
#define CAN_GO(ch, door) \
  (EXIT(ch, door) && real_roomp(EXIT(ch, door)->to_room) && !IS_SET(EXIT(ch, door)->exit_info, EX_CLOSED))

#define CAN_GO_HUMAN(ch, door) \
  (EXIT(ch, door) && real_roomp(EXIT(ch, door)->to_room) && !IS_SET(EXIT(ch, door)->exit_info, EX_LOCKED))
