...
int game_loop(int s) {
  ...
  mask = sigmask(SIGUSR1) | sigmask(SIGUSR2) | sigmask(SIGINT) | sigmask(SIGPIPE) | sigmask(SIGALRM) |
         sigmask(SIGTERM) | sigmask(SIGURG) | sigmask(SIGXCPU) | sigmask(SIGHUP);
    ...
    for (point = descriptor_list; point; point = point->next)
      if ((FD_ISSET(point->descriptor, &output_set) && point->output.head) || (point->prompt_mode)) {
        ...
        if (point->str)
          ...
          if (point->showstr_point || point->pagedfile)
          ...
          else if ((IS_SET(point->character->specials.act, PLR_VT100)) || (IS_SET(point->character->specials.act, PLR_ANSI))) {
            ...
            current_hit = 10 * ((float)GET_HIT(point->character) / (float)hit_limit(point->character));
            current_mana = 10 * ((float)GET_MANA(point->character) / (float)GET_MAX_MANA(point->character));
            current_moves = 10 * ((float)GET_MOVE(point->character) / (float)GET_MAX_MOVE(point->character));
            if (current_hit < 0)
              current_hit = 0;
            if (current_mana < 0)
              current_mana = 0;
            ...
            if ((current_hit <= 2) && IS_SET(point->character->specials.act, PLR_ANSI))
            ...
            if ((current_mana <= 2) && IS_SET(point->character->specials.act, PLR_ANSI))
              SEND_TO_Q(ANSI_RED, point);
            else
              SEND_TO_Q(VT_BOLDTEX, point);
            if (IS_SET(point->character->specials.act, PLR_ANSI))
            ...
            if (IS_SET(point->character->specials.act, PLR_COLOR)) {
              if (IS_IMMORTAL(point->character)) {
                ...
                sprintf(
                  promptbuf, "%sH:%d %sR:%d>%s ", ANSI_CYAN, point->character->points.hit, ANSI_VIOLET, rm->number,
                  ANSI_WHITE
                );
                ...
                prompt_per = point->character->points.hit * 100 / point->character->points.max_hit;
                if (prompt_per < 20) {
                  strcpy(hitscolor, ANSI_RED);
                } else {
                  strcpy(hitscolor, ANSI_GREEN);
                }
                prompt_per = point->character->points.mana * 100 / GET_MAX_MANA(point->character);
                if (prompt_per < 20) {
                  strcpy(manacolor, ANSI_RED);
                } else {
                  strcpy(manacolor, ANSI_VIOLET);
                }
                prompt_per = point->character->points.move * 100 / GET_MAX_MOVE(point->character);
                if (prompt_per < 20) {
                ...
                sprintf(
                  promptbuf, "H:%s%d %sM:%s%d %sV:%s%d%s>%s ", hitscolor, point->character->points.hit, ANSI_WHITE,
                  manacolor, point->character->points.mana, ANSI_WHITE, movescolor, point->character->points.move,
                ...
                sprintf(
                  promptbuf, "H:%s%d %sV:%s%d%s>%s ", hitscolor, point->character->points.hit, ANSI_WHITE, movescolor,
                ...
                prompt_per = point->character->points.move * 100 / point->character->points.max_move;
                if (prompt_per < 20) {
                  strcpy(movescolor, ANSI_RED);
                } else {
                  strcpy(movescolor, ANSI_CYAN);
              ...
              if (GetMaxLevel(point->character) > 50) {
                rm = real_roomp(point->character->in_room);
                sprintf(promptbuf, "H:%d R:%d> ", point->character->points.hit, rm->number);
                write_to_q(promptbuf, &point->output);
              } else if (HasClass(point->character, CLASS_MAGIC_USER) || 
                       HasClass(point->character, CLASS_ANTIPALADIN) ||
                       HasClass(point->character, CLASS_PALADIN) ||
             ...
             HasClass(point->character, CLASS_CLERIC))  {
                sprintf(
                  promptbuf, "H:%d M:%d V:%d> ", point->character->points.hit, point->character->points.mana,
             ...
             HasClass(point->character,CLASS_WARRIOR) ||
                       HasClass(point->character,CLASS_MONK)) {
                sprintf(promptbuf, "H:%d V:%d> ", point->character->points.hit, point->character->points.move);
                write_to_q(promptbuf, &point->output);
              } else {
                sprintf(promptbuf, "*H:%d V:%d> ", point->character->points.hit, point->character->points.move);
                write_to_q(promptbuf, &point->output);
...
void coma(int s) {
  ...
  sigsetmask(
    sigmask(SIGUSR1) | sigmask(SIGUSR2) | sigmask(SIGINT) | sigmask(SIGPIPE) | sigmask(SIGALRM) | sigmask(SIGTERM) |
...
void act(char *str, int hide_invisible, struct char_data *ch, struct obj_data *obj, void *vict_obj, int type) {
  ...
  for (; to; to = to->next_in_room) {
    if (to->desc && ((to != ch) || (type == TO_CHAR)) && (CAN_SEE(to, ch) || !hide_invisible) && !((type == TO_NOTVICT) && (to == (struct char_data *)vict_obj))) {
