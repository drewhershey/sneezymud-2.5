...
# Note: This C project uses /code/ not /code/code/
PROJECT_PATH_PATTERN = "/code/"
PROJECT_GLOB_PATTERN = "*/code/*"
...
def require_compile_commands(preset: str) -> Path:
    ...
    For this project, build output goes to build/ directly (not build/<preset>/).
    """
    ...
    if not compile_commands.exists():
        ...
        print(
            f"Run 'make build' or 'cmake --preset {preset}' first.", file=sys.stderr)
        sys.exit(1)
...
def filter_project_files(issues: list[Issue]) -> list[Issue]:
    """Filter to only issues in project source files (code/).
